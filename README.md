...
```bash
npm install
git config core.hooksPath .githooks   # enable the LinkedIn-URL pre-commit guard
...
The pre-commit hook (in `.githooks/`) blocks accidental commits of real LinkedIn profile URLs to source. Use `linkedin.com/in/example-slug` or `linkedin.com/in/USERNAME` as placeholders.
